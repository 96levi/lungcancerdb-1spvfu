...
export class MutationComponent implements OnInit, OnChanges {
  @Input() stt: number;
  @Input() table: string;
    ...
    this.dataService.checkWorkup('CHECK_WORKUP', this.table, this.stt, this.dateHos).subscribe(res => {
      if (res) {
        ...
        this.dataService.getData('GET_WORKUP_DATA', this.table, this.stt, this.dateHos, this.date).subscribe(formdata => {
          ...
          this.mutationform.setValue(formdata);
        });
      } else {
        this.add_view = 'add';
        this.dataService.getData('GET_WORKUP_DATA', this.table, this.stt, this.dateHos).subscribe(data => {
          this.mutationform.controls.record.setValue(data);
          this.mutationform.controls.stt.setValue(this.stt);
          this.mutationform.controls.dateHos.setValue(new Date(this.dateHos));
          this.mutationform.controls.date.setValue(new Date(this.dateHos));
        });
      }
    });
  }
  ...
  onSubmit() {
    if (this.mutationform.invalid) {
      ...
      const fbData = this.mutationform.value;
      fbData.date = this.mutationform.controls.date.value.getTime(); // Locale date to BigINT
      fbData.dateHos = this.mutationform.controls.dateHos.value.getTime(); // Locale date to BigINT
