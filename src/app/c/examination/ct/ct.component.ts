...
export class CtComponent implements OnInit, OnChanges {
  @Input() stt: number;
  @Input() table: string;
    ...
    this.dataService.checkWorkup('CHECK_WORKUP', this.table, this.stt, this.dateHos).subscribe(res => {
      if (res) {
        ...
        this.dataService.getData('GET_WORKUP_DATA', this.table, this.stt, this.dateHos, this.date).subscribe(formdata => {
          ...
          this.ctform.setValue(formdata);
        });
      } else {
        this.add_view = 'add';
        this.dataService.getData('GET_WORKUP_DATA', this.table, this.stt, this.dateHos).subscribe(data => {
          this.ctform.controls.record.setValue(data);
          this.ctform.controls.stt.setValue(this.stt);
          this.ctform.controls.dateHos.setValue(new Date(this.dateHos));
          this.ctform.controls.date.setValue(new Date(this.dateHos));
        });
      }
    });
  }
  ...
  onSubmit() {
    if (this.ctform.invalid) {
      ...
      const fbData = this.ctform.value;
      fbData.date = this.ctform.controls.date.value.getTime(); // Locale date to BigINT
      fbData.dateHos = this.ctform.controls.dateHos.value.getTime(); // Locale date to BigINT
