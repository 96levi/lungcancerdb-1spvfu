...
export class FollowupComponent implements OnInit, OnChanges {
    ...
    this.dataService.checkWorkup('CHECK_WORKUP', this.table, this.stt, this.dateHos).subscribe(res => {
      if (res) {
        ...
        this.dataService.getData('GET_WORKUP_DATA', this.table, this.stt, this.dateHos, this.date).subscribe(formdata => {
          ...
          this.followupform.setValue(formdata);
        });
      } else {
        this.add_view = 'add';
        this.dataService.getData('GET_WORKUP_DATA', this.table, this.stt, this.dateHos).subscribe(data => {
          this.followupform.controls.record.setValue(data);
          this.followupform.controls.stt.setValue(this.stt);
          this.followupform.controls.dateHos.setValue(new Date(this.dateHos));
          this.followupform.controls.date.setValue(new Date(this.dateHos));
        });
      }
    });
  }
  ...
  onSubmit() {
    if (this.followupform.invalid) {
      ...
      const fbData = this.followupform.value;
      fbData.date = this.followupform.controls.date.value.getTime(); // Locale date to BigINT
      fbData.dateHos = this.followupform.controls.dateHos.value.getTime(); // Locale date to BigINT
